...
def secret_number():
    ...
    rest_of_numbers = random.sample([n for n in all_numbers if n != first_number], 3)
...
def control_tip(tip):
    ...
    if not tip.isdigit():
        ...
        print("The number must contain 4 characters.")
        return False
    elif tip[0] == "0":
...
def evaluate_tip(secret, tip):
    ...
    bulls = sum(tip[i] == secret[i] for i in range(4))
